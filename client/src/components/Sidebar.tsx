...
const Sidebar = ({ isMenuOpen, project, setProject, isGenerating, setIsGenerating }: SideBarProps) => {
    ...
    return (
            <div className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 transition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden '
                : 'w-full'}`}>
                            ...
                            if (isMessage) {
                                ...
                                return (
                                    <div key={msg.id} className={`flex items-center gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
                                        {!isUser && (
                                            <div className='w-8 h-8 shrink-0 rounded-full bg-linear-to-b from-indigo-600 to-indigo-700 flex items-center justify-center'>
                                        ...
                                        {isUser && (
                                            <div className='w-8 h-8 shrink-0 rounded-full bg-gray-700 flex items-center justify-center'>
                                    ...
                                    <div key={ver.id} className='w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                                        ...
                                        <div className='flex items-center justify-between'>
                                            {project.current_version_index === ver.id ? (
                                                ...
                                                <button onClick={() => setRollbackId(ver.id)} className='px-3 py-1 rounded-md text-xs bg-indigo-500 hover:bg-indigo-600 text-white'>
                        ...
                        })}
                        {
                            isGenerating && (
                                <div className='flex items-start gap-3 justify-start'>
                                    <div className='w-8 h-8 shrink-0 rounded-full bg-linear-to-br from-indigo-600 to-indigo-700 flex items-center justify-center'>
                                        <BotIcon className='size-5 text-white' />
                                    </div>
                    ...
                    {rollbackId && (
                        <div className='fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm p-4'>
                            <div className='bg-gray-900 border border-gray-800 p-6 rounded-2xl shadow-2xl max-w-sm w-full animate-in zoom-in-95 duration-200'>
                                <h3 className='text-lg font-semibold text-white mb-2'>Rollback Version?</h3>
                                <p className='text-gray-400 text-sm mb-6'>Are you sure you want to rollback to this version? Your current changes will be replaced.</p>
                                <div className='flex gap-3 justify-end'>
                                    <button onClick={() => setRollbackId(null)} className='px-4 py-2 text-sm font-medium text-gray-400 hover:text-white transition-colors'>
                                        Cancel
                                    </button>
                                    <button onClick={handleRollback} className='px-4 py-2 text-sm font-medium bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors'>
                                        Confirm
                                    </button>
                                </div>
                            </div>
