...
const EditorPanel = ({ selectedElement, onUpdate, onClose }: EditorPanelProps) => {
    ...
    const handleStyleChange = (styleName: string, value: string) => {
        const newValues = { ...values.style, [styleName]: value };
        setValues({ ...values, style: newValues });
            ...
            <div className='space-y-4 text-black'>
                ...
                <div className='grid grid-cols-2 gap-3'>
                    <div>
                        ...
                        <input type='text' value={values.style.padding} onChange={(e) => handleStyleChange('padding', e.target.value)} className='w-full text-sm p-2 border rounded-md border-gray-400 focus:ring-2 focus:ring-indigo-500 outline-none' />
                    </div>
                </div>
                <div className='grid grid-cols-2 gap-3'>
                    <div>
                        <label className='block text-xs font-medium text-gray-500 mb-1'>Margin</label>
                        <input type='text' value={values.style.margin} onChange={(e) => handleStyleChange('margin', e.target.value)} className='w-full text-sm p-2 border rounded-md border-gray-400 focus:ring-2 focus:ring-indigo-500 outline-none' />
                    </div>
                </div>
                <div className='grid grid-cols-2 gap-3'>
                    <div>
                        <label className='block text-xs font-medium text-gray-500 mb-1'>Font Size</label>
                        <input type='text' value={values.style.fontSize} onChange={(e) => handleStyleChange('fontSize', e.target.value)} className='w-full text-sm p-2 border rounded-md border-gray-400 focus:ring-2 focus:ring-indigo-500 outline-none' />
                    </div>
                </div>
                <div className='grid grid-cols-2 gap-3'>
                    <div>
